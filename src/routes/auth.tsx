...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Connexion - CVForge" },
      {
        name: "description",
        content: "Connectez-vous ou créez un compte CVForge pour gérer vos CV ATS.",
      },
    ],
  }),
  component: AuthPage,
...
function getAuthErrorMessage(message?: string) {
  ...
  if (/email not confirmed/i.test(value)) {
    return "Votre email n'est pas encore confirmé. Ouvrez le lien envoyé par email, puis reconnectez-vous.";
  }
  ...
  if (/weak|easy to guess|known to be weak|password.*weak/i.test(value)) {
    return "Ce mot de passe est refusé par Supabase car il est considéré comme trop faible. Le formulaire accepte 8 caractères minimum, mais la règle serveur doit aussi être ajustée dans Supabase Auth.";
  }
  ...
  return value || "Une erreur est survenue. Réessayez dans un instant.";
}
    ...
    <div className="flex min-h-screen items-center justify-center bg-hero px-4 py-12">
      <div className="w-full max-w-md">
        <Link to="/" className="mb-8 flex items-center justify-center gap-2">
          ...
          <span className="font-display text-2xl font-bold">CVForge</span>
        </Link>
...
function SignInForm() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (error) {
      toast.error("Échec de connexion", { description: getAuthErrorMessage(error.message) });
      return;
    }
...
function SignUpForm() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (error) {
      toast.error("Échec d'inscription", { description: getAuthErrorMessage(error.message) });
      return;
    }
    ...
    <form onSubmit={onSubmit} className="mt-4 space-y-3">
      {confirmationEmail && (
        <Alert className="border-emerald-500/30 bg-emerald-500/10 text-emerald-950 dark:text-emerald-50">
          ...
          <AlertTitle>Vérification email requise</AlertTitle>
          <AlertDescription>
            Un lien de confirmation a été envoyé à {confirmationEmail}. Confirmez votre email avant
            de vous connecter.
          </AlertDescription>
        </Alert>
      )}
...
function GoogleButton() {
  ...
  async function onClick() {
    ...
    if (error) {
      toast.error("Échec de connexion Google", { description: getAuthErrorMessage(error.message) });
    }
  }
